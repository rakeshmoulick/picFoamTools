 ...
 * No collision algorithm is attached.
...
double tempN;         // Temperature of the neutral species in eV
double neut_spwt; 
...
// [Write Outputs]
void Write_ts(int ts, Species *neutrals);
...
int parse_ini_file(char * ini_name)
{
    ...
    /*[population]*/
    tempN         = iniparser_getdouble(ini,"population:tempN",-1.0);
    neut_density   = iniparser_getdouble(ini,"population:neut_density",-1.0);
    nn0 = neut_density;
    neut_spwt = iniparser_getdouble(ini,"population:neut_spwt",-1.0);
...
int main(int argc, char *argv[])
{
    ...
    species_list.emplace_back("Neutrals",massN,0,neut_spwt, tempN);
    //species_list.emplace_back("Neutrals", massN, 0, nParticlesN, tempN);

    /*Assign the species list as ions and electrons*/    
    Species &neutrals = species_list[0];

    /*Initiate the species density fields*/    
    neutrals.den = new double[domain.ni];

    neutrals.xvel = new double[domain.ni];
    neutrals.yvel = new double[domain.ni];
    neutrals.zvel = new double[domain.ni];      
    ...
    {
        /*Inject or sample new neutral particle*/
        SampleNeut(&neutrals, tempN, neutrals.mass);       
        //ScatterSpecies(&neutrals);        
	    PushNeutrals(&neutrals);
        //ScatterSpecies(&neutrals);
        if(!steady_state)
		{
			/* add up particles */
			int np_sum=0;
			np_sum += neutrals.part_list.size();
...
void SampleNeut(Species *species, double T, double mass)
{
    ...
    double num_per_sec = nn0*vd*area; 
